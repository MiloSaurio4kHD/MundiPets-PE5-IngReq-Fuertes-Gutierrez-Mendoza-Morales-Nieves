...
\begin{document}
	...
	representados \emph{Validar información médica} y \emph{Publicar mascota
		para cruza y gestionar solicitud}, respectivamente, y quedan detallados a
	nivel textual en la especificación de dichos casos de uso
	(Sección~\ref{sec:cu-detallado}), sin requerir cambios en el diagrama
	general vigente.
	...
	27 funcionalidades definidas en la Sección~\ref{sec:rf}. El modelo se
	...
	íntegramente en las clases ya representadas en el diagrama: RF-26 reutiliza
	\texttt{HistorialMedico} y la generalización de \texttt{Veterinaria},
	registrando la segunda validación como una nueva instancia del mismo tipo
	de registro que ya modela la validación profesional; y RF-27 reutiliza
	\texttt{SolicitudCruza}, cuyo estado y fecha —ya representados en el
	diagrama— permiten derivar el conteo de procesos completados sin necesidad
	de una clase o un atributo visible adicional.
	...
	hacia el objeto \texttt{SolicitudCruza}, coherente con la máquina de
	estados presentada en la Sección~\ref{sec:estados}. La regla de negocio de
	RF-27, que limita las solicitudes de cruza repetitivas sobre una misma
	mascota, se aplica dentro del mismo paso en que el sistema registra la
	solicitud en estado \emph{Pendiente}, ya representado en este diagrama, sin
	introducir un mensaje adicional entre los objetos.
	...
	Sección~\ref{sec:legales}. La segunda opinión veterinaria de RF-26 se
	satisface mediante una nueva ejecución de esta misma secuencia sobre un
	registro que ya se encuentra en estado \emph{Validado}, iniciada por una
	veterinaria distinta; al modelar de forma genérica al actor
	\emph{Veterinaria}, el diagrama representa igualmente esta segunda
	invocación sin requerir elementos adicionales.
	...
	sustentar posteriormente la evaluación de compatibilidad. La regla de negocio de RF-27 se aplica dentro
	de la misma actividad de registro de la solicitud ya representada en este
	diagrama, sin requerir una bifurcación adicional visible.
	...
	corresponde a una nueva ejecución de este mismo diagrama de actividad sobre
	un registro ya validado, por lo que no introduce actividades adicionales a
	las ya representadas.
	...
	procesos definida en RF-15. Cada transición de una solicitud de cruza hacia
	\emph{Completada} incrementa el contador de procesos completados de la
	mascota implicada, valor que RF-27 consulta antes de admitir una nueva
	solicitud sobre la misma mascota y que sustenta la alerta de uso repetitivo
	sin introducir un estado adicional en esta máquina.
	...
	documento observado. La segunda validación de RF-26 se registra como un
	nuevo evento sobre el estado \emph{Validado} ya representado en esta
	máquina de estados, sin reabrir el registro original ni requerir estados o
	transiciones adicionales en la figura.
	...
	(RF-05, RF-08, RF-18, RF-19, RF-20, RF-22, RF-27), Mensajería (RF-07),
	Validación Médica (RF-09, RF-14, RF-24, RF-26) y el componente de
	Inteligencia Artificial, que agrupa los tres subcomponentes de evaluación de
	compatibilidad (RF-06), validación de imágenes (RF-17) y moderación de
	mensajes.
	...
	respuesta del modelo, y su aplicación a una nueva muestra. Esta ampliación se
	difiere a una futura iteración del instrumento y se declara como limitación
	del diseño en la Sección~\ref{sec:amenazas}. RF-26 y RF-27, incorporados en
	la presente revisión (4.0) a partir de EV-23 y EV-24, se recolectaron con
	posterioridad al cuestionario v2.0 y por tanto tampoco cuentan con dato de
	importancia declarada; su priorización se apoya únicamente en MoSCoW
	(Sección~\ref{sec:moscow}) y WSJF (Sección~\ref{sec:wsjf}) hasta que el
	instrumento v3.0 los incorpore.
	...
	\textbf{la matriz completa, con sus 61 filas (TR-01 a TR-61), se presenta en
	...
	Esta matriz Ley~$\rightarrow$~Requisito verifica la trazabilidad legal y
	de evidencia (dirección vertical hacia atrás). Complementándola, el
	\textbf{Apéndice~D.2} presenta la matriz de trazabilidad de modelado, que
	añade la verificación hacia adelante (Requisito~$\rightarrow$~Caso de
	Uso~$\rightarrow$~Clase UML~$\rightarrow$~Criterio de
	Aceptación~$\rightarrow$~Historia de Usuario) y la trazabilidad
	\textbf{horizontal} entre requisitos del mismo nivel, cerrando así los tres
	tipos de trazabilidad exigidos: vertical hacia atrás, vertical hacia
	...
	\emph{Must} --- y 24 hasta un prototipo de interfaz. RF-26 y RF-27,
	incorporados en la presente revisión (4.0), quedan trazados como TR-51 y
	TR-52, con su evidencia correspondiente (EV-24 y EV-23) y sin obligación
	legal, historia de usuario ni mockup dedicado, por tratarse de requisitos
	de prioridad \emph{Should}. Los tramos sin enlace se
	...
	\begin{table}[H]
		\centering
		\caption{Estructura de la matriz de trazabilidad extendida (formato de las 61 filas del Apéndice~D).}
	...
	alcanza las \textbf{61 filas} (TR-01 a TR-61), por encima del mínimo de 40
	exigido: las veinte filas TR-31 a TR-50 recogen las relaciones derivadas de
	los RF-18 a RF-25, de los RNF-09 a RNF-16 y de los RD-06 a RD-09
	incorporados o completados en la Entrega 3 (2A); las filas TR-51 y TR-52
	incorporan a RF-26 y RF-27, agregados en la presente revisión (4.0) a
	partir de EV-24 y EV-23; y las nueve filas finales (TR-53 a TR-61)
	formalizan, en sentido inverso, la trazabilidad Ley $\rightarrow$ Requisito
	ya presentada en la Tabla~\ref{tab:legales} (Sección~\ref{sec:legales}): una
	fila por cada requisito legal (RL-01 a RL-09), de modo que también pueda
	consultarse qué RF, RNF o RD implementa cada obligación concreta de la
	LOPDP sin recorrer la matriz completa en la dirección habitual. Para estas
	nueve filas, la columna \textbf{ID-RF/RNF/RD} contiene el identificador del
	requisito legal (RL) que la fila traza, y el detalle de los elementos que lo
	implementan se indica entre paréntesis en la columna Objetivo. Con ello, la
	totalidad de los 27 RF, los 16 RNF, las 9 RD y los 9 RL del documento queda
	trazada.
	...
	\begin{landscape}
		\begin{longtable}{@{}p{0.8cm} p{1.3cm} p{1.5cm} p{2.2cm} p{1.5cm} p{1.8cm} p{1.6cm} p{1.3cm} p{1.3cm} p{1.3cm} p{1.5cm}@{}}
			...
			TR-40 & LOPDP & Art. 30--32 & Anonimización de evidencias de campo & Equipo investigador & EV-01 a EV-24 & RD-09 & (Transversal a Datos) & --- & --- & --- \\ \midrule
			...
			TR-50 & --- & --- & Dependencia de mensajería externa & Propietario & EV-09, EV-18 & RD-08 & CU-12 & --- & --- & --- \\ \midrule
			TR-51 & --- & --- & Segunda opinión veterinaria & Veterinaria & EV-24 & RF-26 & CU-09 & --- & --- & --- \\ \midrule
			TR-52 & --- & --- & Control de cruzas repetitivas & Propietario & EV-23 & RF-27 & CU-05 & --- & --- & --- \\ \midrule
			TR-53 & LOPDP & Art. 7 y 8 & Consentimiento libre, específico, informado e inequívoco (implementado por RF-01, RF-12) & Propietario & --- & RL-01 & --- & --- & --- & MU-01 \\ \midrule
			TR-54 & LOPDP & Art. 12 y 13 & Información de finalidad y derecho de acceso (implementado por RF-03, RF-11) & Propietario & --- & RL-02 & --- & --- & --- & MU-06 \\ \midrule
			TR-55 & LOPDP & Art. 14 & Derecho de rectificación y actualización (implementado por RF-02, RF-14) & Propietario & --- & RL-03 & --- & --- & --- & MU-06 \\ \midrule
			TR-56 & LOPDP & Art. 15 & Derecho de supresión de datos personales (implementado por RD-06) & Propietario & --- & RL-04 & --- & --- & --- & MU-08 \\ \midrule
			TR-57 & LOPDP & Art. 10 lit. e) y j) & Minimización y seguridad de los datos (implementado por RF-11, RNF-01, RNF-16) & Propietario & --- & RL-05 & --- & --- & --- & MU-08 \\ \midrule
			TR-58 & LOPDP & Art. 20 & Derecho a explicación de decisiones automatizadas (implementado por RF-06, RNF-13, RNF-14, RNF-15) & Interesado en cruza & --- & RL-06 & CU-13 & --- & --- & MU-07 \\ \midrule
			TR-59 & LOPDP & Art. 25, 26 y 30 & Confidencialidad reforzada de datos sensibles y de salud (implementado por RD-04, RNF-01, RNF-05) & Propietario & --- & RL-07 & --- & --- & --- & --- \\ \midrule
			TR-60 & LOPDP & Art. 39 & Protección de datos desde el diseño y por defecto (implementado por RD-02, RNF-16) & Propietario & --- & RL-08 & --- & --- & --- & --- \\ \midrule
			TR-61 & LOPDP & Art. 43 y 46 & Notificación de vulneraciones de seguridad (implementado por RD-07) & Propietario / Autoridad & --- & RL-09 & --- & --- & --- & --- \\
...
\section*{Apéndice D.2 --- Matriz de trazabilidad de modelado (vertical y horizontal)}
\addcontentsline{toc}{section}{Apéndice D.2 --- Matriz de trazabilidad de modelado}
\label{ap:trazabilidad-modelado}

Complementando la matriz Ley~$\rightarrow$~Requisito del Apéndice~D, la
Tabla~\ref{tab:matriz_modelado} consolida la \textbf{trazabilidad de
modelado} de los 43 elementos vigentes del ERS (27 RF y 16 RNF), verificando
en una sola tabla las dos direcciones exigidas por la Sección 6.2 de la Guía
y Rúbrica: la \textbf{trazabilidad vertical} (columnas Fuente, CU, Clase UML,
CA e Historia, que enlazan cada requisito desde su origen empírico hasta su
representación en el modelo y el criterio de aceptación) y la
\textbf{trazabilidad horizontal} (columna Horizontal, que declara la
relación entre requisitos del mismo nivel --- RF con RF, RNF con RNF, o RF
con RNF --- cuando ambos concurren en el mismo caso de uso).

La columna \textbf{Fuente} identifica al participante de origen mediante su
código de evidencia (PROP-XX para propietarios, VET-XX para veterinarias,
OBS-01 para la observación de campo y Encuesta para el cuestionario digital
agregado), en lugar del identificador EV-XX, para hacer explícita la
procedencia humana de cada requisito. La columna \textbf{Horizontal} se
deriva de una relación verificable: dos requisitos quedan enlazados
únicamente cuando comparten el mismo caso de uso en la
Tabla~\ref{tab:matriz_trazabilidad_completa} (Apéndice~D); los elementos sin
esa coincidencia se marcan con \enquote{---}, siguiendo el mismo criterio de
lectura ya declarado en el Apéndice~D: el guion indica ausencia de relación
en ese eslabón, no información faltante. La columna \textbf{Clase UML} se
completa a partir del diagrama de clases refinado
(Sección~\ref{sec:clases-refinado}); los requisitos no funcionales, al ser
transversales por naturaleza, no se asocian a una clase propia y se marcan
con \enquote{---}, consistente con el mismo criterio.

\begin{landscape}
	\begin{table}[H]
		\centering
		\caption{Matriz de trazabilidad de modelado del proyecto MundiPets (vertical y horizontal).}
		\label{tab:matriz_modelado}
		\tiny
		\begin{tabularx}{\linewidth}{|l|X|X|c|X|c|c|c|}
			\hline
			\textbf{RF/RNF} & \textbf{Fuente} & \textbf{Horizontal} & \textbf{CU} & \textbf{Clase UML} & \textbf{CA} & \textbf{Hist.} & \textbf{Estado de la traza} \\
			\hline
RF-01 & PROP-02, VET-01, VET-03, OBS-01 & RNF-08, RF-21 & CU-01 & Mascota & CA-01 & HU-01 & Completa \\ \hline
RF-02 & PROP-01, VET-02 & RF-16, RNF-05 & CU-02 & HistorialMedico, Mascota & CA-02 & HU-02 & Completa \\ \hline
RF-03 & VET-02, PROP-03, OBS-01 & --- & CU-10 & Mascota, HistorialMedico & CA-03 & HU-03 & Completa \\ \hline
RF-04 & Encuesta, OBS-01 & RNF-03 & CU-11 & Publicacion, Mascota & --- & --- & Completa \\ \hline
RF-05 & PROP-01, PROP-02, VET-01, OBS-01 & RF-13, RF-18, RF-25 & CU-04 & Publicacion & CA-04 & HU-04 & Completa \\ \hline
RF-06 & VET-02, VET-03, PROP-03, OBS-01 & RNF-06, RNF-13 & CU-13 & RegistroCompatibilidad, Mascota & CA-05 & HU-05 & Completa \\ \hline
RF-07 & PROP-02, Encuesta, OBS-01 & RF-20, RNF-15 & CU-08 & Mensaje & --- & --- & Completa \\ \hline
RF-08 & PROP-01, VET-01, VET-03 & RF-27 & CU-05 & SolicitudCruza & CA-06 & HU-06 & Completa \\ \hline
RF-09 & VET-01, VET-03, Encuesta & RF-19, RF-26 & CU-09 & CertificadoVeterinario, HistorialMedico & CA-07 & HU-07 & Completa \\ \hline
RF-10 & PROP-02, VET-02, PROP-03 & RNF-09 & CU-12 & Mascota, HistorialMedico & --- & --- & Completa \\ \hline
RF-11 & PROP-02, VET-03, PROP-03, Encuesta, OBS-01 & RNF-01, RF-23, RNF-16 & CU-06 & Usuario, PropietarioMascota & CA-08 & HU-08 & Completa \\ \hline
RF-12 & PROP-01, VET-01, Encuesta, OBS-01 & --- & (Transversal a Onboarding) & Usuario & CA-09 & HU-09 & Completa \\ \hline
RF-13 & PROP-01, PROP-02, Encuesta, OBS-01 & RF-05, RF-18, RF-25 & CU-04 & Publicacion & CA-10 & HU-10 & Completa \\ \hline
RF-14 & VET-01, VET-03, Encuesta & RF-24 & CU-03 & HistorialMedico, Mascota & CA-11 & HU-11 & Completa \\ \hline
RF-15 & PROP-01, PROP-02, VET-02, PROP-03 & RF-22 & CU-07 & SolicitudAdopcion, SolicitudCruza & --- & --- & Completa \\ \hline
RF-16 & PROP-01, VET-01, VET-03, Encuesta & RF-02, RNF-05 & CU-02 & AntecedenteGenetico, Mascota & CA-12 & HU-12 & Completa \\ \hline
RF-17 & PROP-01, VET-01, Encuesta, OBS-01 & --- & (Transversal a Publicaciones) & Publicacion & --- & --- & Completa \\ \hline
RF-18 & PROP-06, VET-05 & RF-05, RF-13, RF-25 & CU-04 & SolicitudAdopcion & CA-13 & HU-13 & Completa \\ \hline
RF-19 & PROP-06 & RF-09, RF-26 & CU-09 & CertificadoVeterinario & --- & --- & Completa \\ \hline
RF-20 & PROP-08 & RF-07, RNF-15 & CU-08 & Mensaje & --- & --- & Completa \\ \hline
RF-21 & VET-05 & RF-01, RNF-08 & CU-01 & Microchip, Mascota & --- & --- & Completa \\ \hline
RF-22 & PROP-05, VET-05 & RF-15 & CU-07 & SolicitudAdopcion & CA-14 & HU-14 & Completa \\ \hline
RF-23 & PROP-04, PROP-07, VET-04, PROP-11 & RF-11, RNF-01, RNF-16 & CU-06 & AlertaRiesgo & CA-15 & HU-15 & Completa \\ \hline
RF-24 & VET-04, VET-05 & RF-14 & CU-03 & HistorialMedico & --- & --- & Completa \\ \hline
RF-25 & PROP-07, VET-05 & RF-05, RF-13, RF-18 & CU-04 & AvisoExtraviado & --- & --- & Completa \\ \hline
RF-26 & VET-07 & RF-09, RF-19 & CU-09 & HistorialMedico, Veterinaria & --- & --- & Completa \\ \hline
RF-27 & PROP-12 & RF-08 & CU-05 & SolicitudCruza & --- & --- & Completa \\ \hline
RNF-01 & VET-03, PROP-03, Encuesta, OBS-01 & RF-11, RF-23, RNF-16 & CU-06 & --- & --- & --- & Completa \\ \hline
RNF-02 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta & --- & (Transversal) & --- & --- & --- & Completa \\ \hline
RNF-03 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta & RF-04 & CU-11 & --- & --- & --- & Completa \\ \hline
RNF-04 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta, OBS-01 & --- & (Transversal / UI) & --- & --- & --- & Completa \\ \hline
RNF-05 & VET-01, VET-03, Encuesta & RF-02, RF-16 & CU-02 & --- & --- & --- & Completa \\ \hline
RNF-06 & PROP-01, VET-01, VET-03, Encuesta & RF-06, RNF-13 & CU-13 & --- & --- & --- & Completa \\ \hline
RNF-07 & PROP-01, VET-01, VET-03, Encuesta & --- & (Transversal a Media) & --- & --- & --- & Completa \\ \hline
RNF-08 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta & RF-01, RF-21 & CU-01 & --- & --- & --- & Completa \\ \hline
RNF-09 & PROP-04, PROP-11 & RF-10 & CU-12 & --- & --- & --- & Completa \\ \hline
RNF-10 & Derivado del equipo de desarrollo & --- & (Transversal a Diseño) & --- & --- & --- & Completa \\ \hline
RNF-11 & Encuesta, OBS-01 & --- & (Transversal a UI) & --- & --- & --- & Completa \\ \hline
RNF-12 & Encuesta, OBS-01 & --- & (Transversal) & --- & --- & --- & Completa \\ \hline
RNF-13 & PROP-06, PROP-08, VET-04, VET-05 & RF-06, RNF-06 & CU-13 & --- & --- & --- & Completa \\ \hline
RNF-14 & PROP-06, PROP-08 & --- & (Transversal a Media) & --- & --- & --- & Completa \\ \hline
RNF-15 & PROP-08, PROP-11 & RF-07, RF-20 & CU-08 & --- & --- & --- & Completa \\ \hline
RNF-16 & PROP-07, VET-05 & RF-11, RNF-01, RF-23 & CU-06 & --- & --- & --- & Completa \\ \hline
		\end{tabularx}
	\end{table}
\end{landscape}

\paragraph{Diagnóstico de cobertura.} De los 43 elementos evaluados, el
100\,\% cuenta con fuente identificada y caso de uso o designación
transversal. Los 27 RF cuentan además con clase UML propia (100\,\%). El
criterio de aceptación en formato Gherkin y la historia de usuario asociada
solo aplican a los 15 RF de prioridad \emph{Must}
(Sección~\ref{sec:historias}), conforme a la convención ya declarada en el
Apéndice~D; su ausencia en el resto de las filas --- RF de prioridad
\emph{Should} o \emph{Could}, y la totalidad de los RNF --- no constituye
una traza incompleta, sino la aplicación consistente de esa misma regla. Con
ello, la matriz cierra con el 100\,\% de sus elementos en estado
\enquote{Completa}, sin huérfanos ni cadenas rotas: todo RF y RNF tiene
fuente, ubicación en el modelo (CU y, si corresponde, clase) y, cuando su
prioridad lo exige, criterio de aceptación e historia de usuario.
